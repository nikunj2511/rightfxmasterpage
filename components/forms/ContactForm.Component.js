...
import { Row, Col, Form, Button, Spinner, Toast } from 'react-bootstrap';
...
export default function ContactForm(props) {
    ...
    const validationSchema = Yup.object({
        ...
        phone_number: Yup.string().matches(phoneRegExp, t('Phone number is not valid')),
                ...
                {({
                    ...
                    return (
                        <Form noValidate onSubmit={handleSubmit}>
                            ...
                            <Row>
                                <Form.Group as={Col} xs="12" md="6" className="position-relative mb-3">
                                    <Form.Select size="lg" name="country" value={values.country} onChange={handleChange} onBlur={handleBlur} isInvalid={touched.country && !!errors.country} required>
                                        <option value="">{t('Select Country')}</option>
                                        {COUNTRIES.map((cc, key) =>
                                            <option key={key} value={cc.countryEn}>{cc.countryEn}</option>
                                        )}
                                    </Form.Select>
                                    <Form.Control.Feedback type="invalid" tooltip>
                                        {errors.country}
